...
void tcsVTxy_c ( double xtar, double ytar, double ztar,
                 double spm1[3][3],
...
**  the position of the image in the focal plane.  The target is
...
**
**  Returned:
**     xim      double        image x-coordinate (Note 6)
**     yim      double        image y-coordinate (Note 6)
**     j        int           status:    0 = OK
...
**
**  6  The image coordinates returned, xim and yim, are in units of one
...
**
**  7  A common application for this routine is autoguiding.  The target
...
/* Get the image [xi,eta] in the non-rotating focal plane. */
   tcsVTxe_c ( xtar, ytar, ztar,
               spm1, frame, sst, cst, spm2, enca, encb,
