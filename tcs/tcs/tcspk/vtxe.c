...
void tcsVTxe ( double tara, double tarb,
               double spm1[3][3],
...
**
**  Returned:
**     xiim     double        image x-coordinate (Note 5)
**     etaim    double        image y-coordinate (Note 5)
**     j        int           status:    0 = OK
...
**
**  5  The image coordinates returned, xiim and etaim, are in the
...
**
**  6  A common application for this routine is controlling a tip/tilt
**     subreflector.
**
...
/* Target coordinates, spherical to Cartesian. */
   cosb = cos ( tarb );
   xtar = cos ( tara ) * cosb;
   ytar = sin ( tara ) * cosb;
   ztar = sin ( tarb );
...
/* Target and encoders to pointing-origin (non-rotating). */
   tcsVTxe_c ( xtar, ytar, ztar, spm1, frame, sst, cst, spm2,
               enca, encb, ia, ib, np, xt, yt, zt, ga, gb,
               xiim, etaim, j );
