...
void tcsVTsky ( double enca, double encb, ROTLOC rotl, double rma,
...
**
**  Returned:
**     tara     double*       target "roll" coordinate (Note 2)
**     tarb     double*       target "pitch" coordinate (Note 2)
**
**  Defined in tcs.h:
...
**     from the mount's encoder readings, as shown by the arrows.
**
...
/* Encoders to sky, result as x,y,z. */
   tcsVTsky_c ( enca, encb, rotl, rma, xpo, ypo,
...
/* To spherical. */
   r = sqrt ( x * x + y * y );
   *tara = ( r != 0.0 ) ? atan2 ( y, x ) : 0.0;
   *tarb = ( z != 0.0 ) ? atan2 ( z, r ) : 0.0;
