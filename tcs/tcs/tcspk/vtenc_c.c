...
void tcsVTenc_c ( double xtar, double ytar, double ztar,
                  double spm1[3][3],
...
**  - - - - - - - - - - -
**
**  In a "virtual telescope", for the specified target and pointing-
**  origin generate the mount encoder demands.  The target is specified
**  in Cartesian form.
**
**  Given:
...
**
**  Returned:
**     enca1    double*       roll, first solution (Note 9)
**     encb1    double*       pitch, first solution (Note 9)
**     enca2    double*       roll, second solution (Note 9)
**     encb2    double*       pitch, second solution (Note 9)
...
**
**  5  The predicted rotator angle rotap is the expected position of
...
**
**  9  If the mount is an altazimuth, a is pi-azimuth and b is
**     elevation.  If the mount is an equatorial, a is -HA and b is
**     declination.
**
...
**     (see Note 8).
**
**  11 This routine is optimized for speed, and accordingly no
...

{
   ...
   tcsTrack_c ( xtar, ytar, ztar,
                spm1, frame, sst, cst, spm2,
                rotl, rotap, ap, bp, xim, yim,
                ia, ib, np, xt, yt, zt, ga, gb, rnogo,
                &xa, &ya, &za, enca1, encb1, enca2, encb2, j );
