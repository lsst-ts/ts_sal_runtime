...
set hexapodCommander2 		$env(SAL_WORK_DIR)/hexapod/cpp/src/sacpp_hexapod_move_commander
...
set rotatorCommander     	$env(SAL_WORK_DIR)/dome/cpp/src/sacpp_rotator_commander

puts stdout "----- Start of visit -----------------------------------------"
if { $argc > 2 } {
   set filter [lindex $argv 2]
   puts stdout "Filter change required"
   puts stdout "Moving rotator to 0.0"
   set status [catch {exec $rotatorCommander 180 target position set absolute "angle=0.0 velocity=0.0 tai=[expr [clock milliseconds]/1000.0]"} result]
